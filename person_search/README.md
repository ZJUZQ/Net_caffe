...
3. Build the Cython modules
  ...
  Install some Python packages you might not have: `Cython`, `python-opencv`, `easydict (>=1.6)`, `PyYAML`, `protobuf`, `mpi4py`. 

  note: before make, maybe need to add " export CUDAHOME=/usr/local/cuda-8.0 " to ~/.bashrc. Then:
  ```Shell
  cd lib && make && cd ..
  ```
