...
%%          maks_miss:  those segmentations of peoples in a image with (num_keypoints == 0) 

addpath('/home/zq/dataset/COCO/coco/MatlabAPI/');
addpath('../testing/util');

mkdir('/home/zq/dataset/COCO/mask2017')
vis = 0;

for mode = 1
    
    if mode == 1 % train
        load('/home/zq/dataset/COCO/mat/coco_kpt.mat');
    else % validation
    ...
    for i = 1:L
        if mode == 1
            ...
            img_name1 = sprintf('/home/zq/dataset/COCO/mask2017/train2017_mask_all_%012d.png', coco_kpt(i).image_id);
            img_name2 = sprintf('/home/zq/dataset/COCO/mask2017/train2017_mask_miss_%012d.png', coco_kpt(i).image_id);
        else
        ...
        try
            ...
            imread(img_name1);
            imread(img_name2);
            continue; %% if img_name1 and img_name2 already exist, continue
        catch
            display([num2str(i) '/ ' num2str(L)]);
            %joint_all(count).img_paths = RELEASE(i).image_id;
            [h,w,~] = size(imread(['/home/zq/dataset/COCO/', img_paths]));
            mask_all = false(h,w);
            mask_miss = false(h,w);
            flag = 0;
            for p = 1:length(coco_kpt(i).annorect)
                ...
                try
                    seg = coco_kpt(i).annorect(p).segmentation{1}; %% iscrowd == 0, segmentation is [polygon], segmentation of one people may contains several polygon
                catch
                ...
                mask = inpolygon( X, Y, seg(1:2:end), seg(2:2:end)); %% Points located inside or on edge of polygonal region
            ...
            if flag == 1 %% has a crowd segmentation
                %% crowd segmentation is added to mask_miss and mask_all together
            ...
            if mode == 1
                img_name = sprintf('/home/zq/dataset/COCO/mask2017/train2017_mask_all_%012d.png', coco_kpt(i).image_id);
                imwrite(mask_all,img_name);
                img_name = sprintf('/home/zq/dataset/COCO/mask2014/train2014_mask_miss_%012d.png', coco_kpt(i).image_id);
            ...
            if flag == 1 && vis == 1
                im = imread(['/home/zq/dataset/COCO/', img_paths]);
    ...
    if mode == 1 
        save('/home/zq/dataset/COCO/mat/coco_kpt_mask.mat', 'coco_kpt', '-v7.3');
    else
        coco_val = coco_kpt;
