...
class Layer {
 public:
  ...
  explicit Layer(const LayerParameter& param)
    : layer_param_(param), is_shared_(false) {
      ...
      phase_ = param.phase(); //  phase: TRAIN or TEST
      if (layer_param_.blobs_size() > 0) {
        blobs_.resize(layer_param_.blobs_size());
        for (int i = 0; i < layer_param_.blobs_size(); ++i) {
          blobs_[i].reset(new Blob<Dtype>());
          blobs_[i]->FromProto(layer_param_.blobs(i)); // 申请空间，然后将传入的layer_param中的blob拷贝过来
        }
      }
    }
  virtual ~Layer() {}
 ...
 private:
  ...
  shared_ptr<boost::mutex> forward_mutex_; // The mutex class implements the Lockable concept of Boost
