...
class CPMDataTransformer {
 public:
  ...
  // generate gaussian Maps and vec Maps
  void generateLabelMap(Dtype* transformed_label, Mat& img_aug, MetaData meta);
  ...
  bool onPlane(Point p, Size img_size); // judge Point p in range of img_size
  void swapLeftRight(Joints& j); // swap left joint points' and right joint points' coordinate 
 ...
 protected:
  ...
  // read metadata from data, very specific to genLMDB.py
  void ReadMetaData(MetaData& meta, const string& data, size_t offset3, size_t offset1);
  
  //transform joints in meta from np_in_lmdb (17, specified in prototxt) to np (specified in prototxt)
  void TransformMetaJoints(MetaData& meta);
  void TransformJoints(Joints& joints);
  void clahe(Mat& img, int, int); // Contrast-limited Adaptive Histogram Equalization (CLAHE)

  // put gaussian map around center
