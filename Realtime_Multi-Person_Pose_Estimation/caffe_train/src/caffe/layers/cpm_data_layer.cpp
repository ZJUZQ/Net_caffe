...
void CPMDataLayer<Dtype>::load_batch(Batch<Dtype>* batch) {
  ...
  {
    Datum& datum = *(reader_.full().peek()); // Read a data point
    if (datum.encoded()) {
      if (force_color) {
  ...
  for (int item_id = 0; item_id < batch_size; ++item_id) 
  {
    ...
    timer.Start();
    const int offset_data = batch->data_.offset(item_id); // ((n * channels() + c) * height() + h) * width() + w
    ...
    if (datum.encoded()) {
      this->cpm_data_transformer_->Transform(cv_img, &(this->transformed_data_)); // why no transformed_label_ ?
    } 
    else {
      this->cpm_data_transformer_->Transform_nv(datum, &(this->transformed_data_), &(this->transformed_label_), cnt);
      ++cnt;
    }
    // if (this->output_labels_) {
