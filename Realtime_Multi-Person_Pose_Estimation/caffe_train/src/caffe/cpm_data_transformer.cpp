...
void CPMDataTransformer<Dtype>::ReadMetaData(MetaData& meta, const string& data, size_t offset3, size_t offset1) { 
  ...
  meta.joint_self.joints.resize(np_in_lmdb); // coco dataset, np_in_lmdb = 17
...
template<typename Dtype> 
CPMDataTransformer<Dtype>::CPMDataTransformer(const CPMTransformationParameter& param, Phase phase) : param_(param), phase_(phase) {
...
void CPMDataTransformer<Dtype>::Transform_nv(const Datum& datum, Dtype* transformed_data, Dtype* transformed_label, int cnt) {
  ...
  //color, contract
  if(param_.do_clahe()) // Contrast-limited Adaptive Histogram Equalization (CLAHE)
...
Size CPMDataTransformer<Dtype>::augmentation_croppad(Mat& img_src, Mat& img_dst, MetaData& meta) {
  ...
  for(int i=0;i<crop_y;i++){
    for(int j=0;j<crop_x;j++){ //i,j on cropped
      int coord_x_on_img = center.x - crop_x/2 + j;
      int coord_y_on_img = center.y - crop_y/2 + i;
      // because center is perterbed, Point(coord_x_on_img, coord_y_on_img) may out of original image
      if(onPlane(Point(coord_x_on_img, coord_y_on_img), Size(img_src.cols, img_src.rows))){
        img_dst.at<Vec3b>(i,j) = img_src.at<Vec3b>(coord_y_on_img, coord_x_on_img);
      }
    }
...
bool CPMDataTransformer<Dtype>::augmentation_flip(Mat& img_src, Mat& img_aug, MetaData& meta) {
  ...
  if(doflip){
    flip(img_src, img_aug, 1); // cv::flip, flipping around y-axis
    int w = img_src.cols;
...
void CPMDataTransformer<Dtype>::putGaussianMaps(Dtype* entry, Point2f center, int stride, int grid_x, int grid_y, float sigma){
  ...
  for (int g_y = 0; g_y < grid_y; g_y++){
    for (int g_x = 0; g_x < grid_x; g_x++){
      ...
      float exponent = d2 / 2.0 / sigma / sigma; // exp( -(x-u)^2 / (2*sigma^2) )
...
void CPMDataTransformer<Dtype>::putVecMaps(Dtype* entryX, Dtype* entryY, Mat& count, Point2f centerA, Point2f centerB, int stride, int grid_x, int grid_y, float sigma, int thre){
  //int thre = 4;
  centerB = centerB*0.125; // divide by stride
  centerA = centerA*0.125;
  Point2f bc = centerB - centerA;
  ...
  for (int g_y = min_y; g_y < max_y; g_y++){
    for (int g_x = min_x; g_x < max_x; g_x++){
      ...
      ba.x = g_x - centerA.x;
      ba.y = g_y - centerA.y;
      float dist = std::abs(ba.x*bc.y -ba.y*bc.x); // use the formula: distance of point to line
...
void CPMDataTransformer<Dtype>::generateLabelMap(Dtype* transformed_label, Mat& img_aug, MetaData meta) {
    ...
    for(int i=0;i<19;i++){
      ...
      for(int j = 0; j < meta.numOtherPeople; j++){ //for every other person
        ...
        if(jo2.isVisible[mid_1[i]-1]<=1 && jo2.isVisible[mid_2[i]-1]<=1){
          //putVecPeaks
          // selfPeople and otherPeoples use the same paf vecMap of a limb
          putVecMaps(transformed_label + (np+ 1+ 2*i)*channelOffset, transformed_label + (np+ 2+ 2*i)*channelOffset, 
  ...
  if(1 && param_.visualize()){
    ...
    for(int i = 0; i < 2*(np+1); i++){      
      ...
      cv::resize(label_map, label_map, Size(), stride, stride, INTER_LINEAR);
      cv::applyColorMap(label_map, label_map, COLORMAP_JET);
      cv::addWeighted(label_map, 0.5, img_aug, 0.5, 0.0, label_map); // cv_addWeighted, Calculates the weighted sum of two arrays
...
// used in cpm_data_layer.cpp for encoded datum
template<typename Dtype>
...
void CPMDataTransformer<Dtype>::clahe(Mat& bgr_image, int tileSize, int clipLimit) {
  Mat lab_image;
  cvtColor(bgr_image, lab_image, CV_BGR2Lab); //  Lab color space

  // Extract the L channel
  vector<Mat> lab_planes(3);
  split(lab_image, lab_planes);  // now we have the L image in lab_planes[0].   Divides a multi-channel array into several single-channel arrays.

  // apply the CLAHE algorithm to the L channel
  // 对比度有限的自适应直方图均衡（CLAHE）: Contrast-limited Adaptive Histogram Equalization (CLAHE)
