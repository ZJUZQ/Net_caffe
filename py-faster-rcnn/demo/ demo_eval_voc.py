...
def parse_args():
    ...
    parser.add_argument('det_res', 
                        help='Detection result file. Format: img_id,label,confidence,left,top,right,bottom ...')
...
def readDET(eles):
    det = {}
    obj_num = (len(eles)-1)/6
    for i in xrange(obj_num):
        obj = eles[i*6+1:(i+1)*6+1]
        name = getClassName(int(obj[4]))
        if name not in det:
            det[name] = []
        det[name].append([obj[5], obj[0], obj[1], obj[2], obj[3]])
    for key in det:
...
def main():
    ...
    if not os.path.exists(train_model):
        raise IOError("Train model not exist: %s"%(train_model))
    if not os.path.exists(path_in):
        raise IOError("Detection result file not exist: %s"%(path_in)) 
    
    ### Init the benchmark
    train_model = edict(yaml.load(open(train_model, 'r')))
    CLASS_MAP.update(train_model.CLASS)
    benchmark = {}
    ...
    for line in lines:
        ...
        ### Read ground truth
        path, img = os.path.split(eles[0])
        path, folder = os.path.split(path)
        img, ext = os.path.splitext(img)
        ann_path = "%s/Annotations/%s/%s.xml"%(path, folder, img)
        ann = readANN(ann_path)
    ...
    if plt.get_backend().lower() == 'agg':
        fig_ap.savefig("%s_AP.png"%(path_in),  
                       bbox_extra_artists=(legend_ap,),
                       bbox_inches='tight')
        fig_pr.savefig("%s_PR.png"%(path_in),  
