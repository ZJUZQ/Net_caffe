...
void CPMDataTransformer<Dtype>::TransformJoints(Joints& j) {
  ...
  if(np == 56){
    ...
    int head_to_ours_1[4] = {0,1,2,3};
    int head_to_ours_2[4] = {0,1,2,3};
    jo.joints.resize(np);
    jo.isVisible.resize(np);
    for(int i=0;i<4;i++){
      jo.joints[i] = (j.joints[head_to_ours_1[i]] + j.joints[head_to_ours_2[i]]) * 0.5;
      if(j.isVisible[head_to_ours_1[i]]==2 || j.isVisible[head_to_ours_2[i]]==2){
        jo.isVisible[i] = 2;
      }
      else {
        jo.isVisible[i] = j.isVisible[head_to_ours_1[i]] && j.isVisible[head_to_ours_2[i]];
      }
    }
  }
...
float CPMDataTransformer<Dtype>::augmentation_scale(Mat& img_src, Mat& img_temp, Mat& mask_miss, Mat& mask_all, MetaData& meta, int mode) {
  ...
  // disable scale augmentation when in disable_scale dataset 
  float scale_abs = 0;
  string::size_type findP = meta.dataset.find("disable_scale");
  if (findP != meta.dataset.npos)
      scale_abs = 0/meta.scale_self;
  else
      scale_abs = param_.target_dist()/meta.scale_self;
  
  float scale = scale_abs * scale_multiplier;

  // disable scale augmentation if target_dist=0
  if (scale==0)
      scale=1;
...
Size CPMDataTransformer<Dtype>::augmentation_croppad(Mat& img_src, Mat& img_dst, Mat& mask_miss, Mat& mask_miss_aug, Mat& mask_all, Mat& mask_all_aug, MetaData& meta, int mode) {
  ...
  if (center.x-crop_x/2<0) center.x = crop_x/2;
  if (center.x+crop_x/2>=img_src.cols) center.x = img_src.cols-crop_x/2-1;
  if (center.y-crop_y/2<0) center.y = crop_y/2;
  if (center.y+crop_y/2>=img_src.rows) center.y = img_src.rows-crop_y/2-1;
...
void CPMDataTransformer<Dtype>::generateLabelMap(Dtype* transformed_label, Mat& img_aug, MetaData meta) {
  ...
  if (np == 56){
    ...
    for (int i = 0; i < 4; i++){
      Point2f center = meta.joint_self.joints[i];
      if(meta.joint_self.isVisible[i] <= 1){
        putGaussianMaps(transformed_label + (i+np+7)*channelOffset, center, param_.stride(), 
                        grid_x, grid_y, param_.sigma()); //self
      }
      for(int j = 0; j < meta.numOtherPeople; j++){ //for every other person
        Point2f center = meta.joint_others[j].joints[i];
