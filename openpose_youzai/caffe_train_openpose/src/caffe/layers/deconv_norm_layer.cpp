...
void DeconvolutionNormLayer<Dtype>::compute_output_shape() {
...
template <typename Dtype>
void DeconvolutionNormLayer<Dtype>::Reshape(const vector<Blob<Dtype>*>& bottom,
...
template <typename Dtype>
void DeconvolutionNormLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top) {
  ...
  for (int i = 0; i < bottom.size(); ++i) {
    ...
    for (int n = 0; n < this->num_; ++n) {
      ...
      this->backward_cpu_gemm(bot_quan_data, weight,
                              top_dequan_data);
      // dequantize to float
...
template <typename Dtype>
void DeconvolutionNormLayer<Dtype>::Backward_cpu(const vector<Blob<Dtype>*>& top,
...
#ifdef CPU_ONLY
STUB_GPU(DeconvolutionNormLayer);
#endif

INSTANTIATE_CLASS(DeconvolutionNormLayer);
REGISTER_LAYER_CLASS(DeconvolutionNorm);
