...
#include <algorithm>
#include <cfloat>
#include "caffe/layers/depthwise_conv_norm_layer.hpp"
...
__global__ void ConvForward(const int nthreads,
		...
		Dtype* const top_data,const Dtype* const weight,const Dtype* const bias,const bool bias_term_, float int_scale) {
...
//		}
		...
		for (int h = hstart; h < hend; ++h) {
			for (int w = wstart; w < wend; ++w) {
                //quantize to uint8
                Dtype input = int(bottom_slice[h * width + w]*int_scale+0.5f);
                input = input<0? 0:input;
                input = input>255? 255:input;
				aveval += input*weight_slice[(khstart+h-hstart) * kernel_w + (kwstart+w-wstart)];
//				if (index==1) {
//					printf("pos:h%d w%d\n",h,w);
...
template<typename Dtype>
void DepthwiseConvolutionNormLayer<Dtype>::Forward_gpu(
	...
	for (int i = 0; i < bottom.size(); ++i) {
		...
		if (bias_term_) {
			...
			ConvForward<Dtype><<<CAFFE_GET_BLOCKS(count), CAFFE_CUDA_NUM_THREADS>>>(
					...
					kernel_w_, stride_h_, stride_w_, pad_h_, pad_w_, top_data,weight,bias,bias_term_, this->int_scale_);
		} else {
			ConvForward<Dtype><<<CAFFE_GET_BLOCKS(count), CAFFE_CUDA_NUM_THREADS>>>(
					count, bottom_data, bottom[i]->num(), channels_,
					height_, width_,conved_height,conved_weight,kernel_h_,
					kernel_w_, stride_h_, stride_w_, pad_h_, pad_w_, top_data,weight,0,bias_term_, this->int_scale_);
		}
	}
}
...
const Dtype* const bottom_data, float int_scale) {
...
//			
			for(int ph=phstart;ph<phend;ph++){
				for (int pw=pwstart;pw<pwend;pw++){
                    ...
                    Dtype input = int(bottom_data_slice[h*width+w]*int_scale+0.5f);
                    input = input<0? 0:input;
                    input = input>255? 255:input;
                    input = input/int_scale;
					gradient+=top_diff_slice[ph * conved_width + pw]*input;
//					if (index==5) {
...
template<typename Dtype>
void DepthwiseConvolutionNormLayer<Dtype>::Backward_gpu(
...
INSTANTIATE_LAYER_GPU_FUNCS (DepthwiseConvolutionNormLayer);
