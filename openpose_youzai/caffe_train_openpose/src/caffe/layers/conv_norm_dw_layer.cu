...
__global__ void ConvolutionDepthwiseWeightForward(const int nthreads,
    ...
    Dtype* const top_data, float int_scale) {
  CUDA_KERNEL_LOOP(index, nthreads) {
    ...
    for (int kh = 0; kh < kernel_h; ++kh)
    {
      for (int kw = 0; kw < kernel_w; ++kw)
      {
        ...
        if ((h_in >= 0) && (h_in < bottom_height) && (w_in >= 0) && (w_in < bottom_width))
        {
          ...
          //quantize to uint8
          Dtype input = int(bottom_data[offset]*int_scale+0.5f);
    ...
    //dequantize to float
    top_data[index] = value/int_scale;
  }
}
...
template <typename Dtype>
void ConvolutionNormDepthwiseLayer<Dtype>::Forward_gpu(const vector<Blob<Dtype>*>& bottom,
  ...
  ConvolutionDepthwiseWeightForward<Dtype><<<CAFFE_GET_BLOCKS(count), CAFFE_CUDA_NUM_THREADS>>>(
      ...
      pad_h_, pad_w_, dilation_h_, dilation_w_, top_data, int_scale_);
...
__global__ void ConvolutionDepthwiseWeightBackward(const int nthreads,
    ...
    const int kernel_h, const int kernel_w, const int stride_h, const int stride_w,
    const int pad_h, const int pad_w, const int dilation_h, const int dilation_w,
    Dtype* const buffer_data, float int_scale) {
  CUDA_KERNEL_LOOP(index, nthreads) {
    const int h = (index / top_width) % top_height;
    const int w = index % top_width;
    ...
    if ((h_in >= 0) && (h_in < bottom_height) && (w_in >= 0) && (w_in < bottom_width))
    {
      ...
      Dtype input = int(bottom_data[bottom_offset]*int_scale+0.5f);
      input = input<0? 0:input;
      input = input>255? 255:input;
      input = input/int_scale;
      buffer_data[index] = top_diff[top_offset] * input;
    }
    else
    {
      buffer_data[index] = 0;
...
template <typename Dtype>
void ConvolutionNormDepthwiseLayer<Dtype>::Backward_gpu(const vector<Blob<Dtype>*>& top,
  ...
  if (this->param_propagate_down_[0])
  {
    ...
    ConvolutionDepthwiseWeightBackward<Dtype><<<CAFFE_GET_BLOCKS(weight_buffer_count), CAFFE_CUDA_NUM_THREADS>>>(
        ...
        top_height, top_width, bottom_height, bottom_width,
        kernel_h_, kernel_w_, stride_h_, stride_w_,
        pad_h_, pad_w_, dilation_h_, dilation_w_, weight_buffer_mutable_data, int_scale_);
...
INSTANTIATE_LAYER_GPU_FUNCS(ConvolutionNormDepthwiseLayer);
