...
#include "caffe/layers/conv_norm_dw_layer.hpp"

namespace caffe {

template <typename Dtype>
void ConvolutionNormDepthwiseLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top) {
  ConvolutionParameter conv_param = this->layer_param_.convolution_param();
  int_scale_ = 255.0f;
  if (conv_param.has_int_scale())
    int_scale_ = conv_param.int_scale();
  this->inq_partition_ = 0;
  if (conv_param.has_inq_partition())
    this->inq_partition_ = conv_param.inq_partition();
...
template <typename Dtype>
void ConvolutionNormDepthwiseLayer<Dtype>::Reshape(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top) {
...
template <typename Dtype>
void ConvolutionNormDepthwiseLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top)
{
          ...
          for (int kh = 0; kh < kernel_h_; ++kh)
          {
            for (int kw = 0; kw < kernel_w_; ++kw)
            {
              ...
              if ((h_in >= 0) && (h_in < bottom_height) && (w_in >= 0) && (w_in < bottom_width))
              {
                ...
                //quantize to uint8
                Dtype input = int(bottom_data[offset]*int_scale_+0.5f);
                input = input<0? 0:input;
                input = input>255? 255:input;
                value += (*weight_data) * input;
              }
              ++weight_data;
            }
          }
          //dequantize to float
          *top_data++ = value/int_scale_;
        }
      }
    }
  }
...
template <typename Dtype>
void ConvolutionNormDepthwiseLayer<Dtype>::Backward_cpu(const vector<Blob<Dtype>*>& top,
            ...
            for (int kh = 0; kh < kernel_h_; ++kh)
            {
              for (int kw = 0; kw < kernel_w_; ++kw)
              {
                ...
                if ((h_in >= 0) && (h_in < bottom_height) && (w_in >= 0) && (w_in < bottom_width))
                {
                  ...
                  input = input<0? 0:input;
                  input = input>255? 255:input;
                  input = input/int_scale_;
                  *weight_diff += input * (*top_diff);
                }
                ++weight_diff;
              }
            }
...
#ifdef CPU_ONLY
STUB_GPU(ConvolutionNormDepthwiseLayer);
#endif

INSTANTIATE_CLASS(ConvolutionNormDepthwiseLayer);
REGISTER_LAYER_CLASS(ConvolutionNormDepthwise);
