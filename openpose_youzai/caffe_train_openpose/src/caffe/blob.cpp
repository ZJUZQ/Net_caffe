...
template <typename Dtype>
void Blob<Dtype>::FromProto(const BlobProto& proto, bool reshape, bool is_quantization, float inq_partition) {
  if (reshape) {
    vector<int> shape;
  ...
  // INQ
  if(is_quantization && inq_partition>0)
  {
    ...
    //quantizate the top (inq_partition) of each layer, change the "partition" until partition=0
    int partition=int(count_*(1-inq_partition))-1;
    std::cout<<"INQ information: partion_"<<inq_partition<<" , n1_"<<n1<<" , max_value_"<<max_data<<std::endl;
    //set n1 to 1
    //n1 = 1;
